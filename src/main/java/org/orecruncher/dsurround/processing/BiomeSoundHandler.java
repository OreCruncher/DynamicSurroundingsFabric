...
public final class BiomeSoundHandler extends ClientHandler {
    ...
    private void generateBiomeSounds() {
        ...
        for (final Reference2IntMap.Entry<Biome> kvp : this.biomes.getBiomes().reference2IntEntrySet()) {
            final Collection<SoundEvent> acoustics = BiomeLibrary.findSoundMatches(kvp.getKey());
        ...
        if (player.isAlive()) {
            ...
            */
            ...
            if (biomeSounds) {
                ...
                final SoundEvent sound = BiomeLibrary.getRandomSoundAddition(playerBiome, RANDOM);
                if (sound != null) {
                    SoundInstance instance = SoundFactory.createNear(sound, player, SPOT_SOUND_MIN_RANGE, SPOT_SOUND_MAX_RANGE);
