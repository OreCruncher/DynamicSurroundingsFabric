...
public class BiomeVariables extends VariableSet<IBiomeVariables> implements IBiomeVariables {
    ...
    private final Lazy<String> precipitationType = new Lazy<>(() -> {
        ...
        return this.biome.getPrecipitationAt(pos, GameUtils.getWorld().map(w -> w.getSeaLevel()).orElse(63)).name();
    });
    ...
    public float getTemperature() {
        return BiomeCompat.getTemperature(this.biome, GameUtils.getPlayer().map(p -> p.blockPosition()).orElse(null));
    }
